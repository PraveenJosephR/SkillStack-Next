...
export function Sidebar() {
  ...
  const [draftedActivities, setDraftedActivities] = useAtom(
    draftedActivitiesAtom,
  );
  ...
  const totalTokens = draftedActivities.reduce(
    (sum, a) => sum + a.tokensEach,
    0,
  );

  // ─── Get Activities ─────────────────────────────────────────────
  const [activities, setActivities] = useState<
    { id: number; name: string; tokensEach: number }[]
  >([]);
  ...
  async function handleSavePlan() {
    ...
    try {
      ...
      const existingActivityIds = new Set(
        existingGoals.map((g: any) => g.activity_id),
      );
      ...
      const draftedActivityIds = new Set(
        activitiesPayload.map((a) => a.activity_id),
      );
      const goalIdsToDelete = existingGoals
        .filter(
          (g: any) => g.activity_id && !draftedActivityIds.has(g.activity_id),
        )
        .map((g: any) => g.id);
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent className="flex flex-col sm:max-w-lg p-0">
        <SheetHeader className="p-6 pb-4 pr-12">
          ...
          <SheetDescription>
            Select activities and set how many of each you plan to complete this
            semester.
          </SheetDescription>
        </SheetHeader>
              ...
              draftedActivities.map((draft) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <Badge
                      variant="secondary"
                      className="px-1.5 py-0 shadow-none text-[10px]"
                    >
                      {months.find((x) => x.value === draft.month)?.name ||
                        String(draft.month)}
                    </Badge>
          ...
          <div className="flex flex-col gap-3">
            ...
            {totalTokens < 16 && (
              <div className="text-sm text-destructive bg-destructive/10 rounded-md p-2 text-center">
                Minimum 16 tokens required. Add {16 - totalTokens} more tokens
                to save your plan.
              </div>
            )}
          </div>
